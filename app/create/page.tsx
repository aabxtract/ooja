...
export default function CreateBetPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const target = BigInt(Math.round(parseFloat(targetPrice) * 1e8));
      const stakeMicro = BigInt(Math.round(parseFloat(stake) * 1e6));
      const expiryBlocks = BigInt(
        futureExpiryBlocks(Number(expiryMinutes) * 60)
      );
